...
def analisar_valores(lista):
    valores_positivos = [x for x in lista if x > 0]  # Filtra apenas os valores positivos

    if valores_positivos:
        total = sum(lista)
        media_positivos = sum(valores_positivos) / len(valores_positivos)
        maior_valor = max(lista)
        menor_valor = min(valores_positivos)
    else:
        total = sum(lista)
        media_positivos = maior_valor = 0
        menor_valor = min(lista) if any(lista) else 0  # Se houver algum valor, pega o menor, senão, fica como zero
